...
readonly LB_MODEL_ID_FILE="/etc/slm-copilot/lb_model_id"
readonly LB_DEREGISTER_SCRIPT="/usr/local/bin/slm-copilot-lb-deregister"
readonly LB_DEREGISTER_UNIT="/etc/systemd/system/slm-copilot-lb-deregister.service"
readonly LB_HEALTHCHECK_SCRIPT="/usr/local/bin/slm-copilot-lb-healthcheck"
readonly LB_HEALTHCHECK_UNIT="/etc/systemd/system/slm-copilot-lb-healthcheck.service"
readonly LB_HEALTHCHECK_TIMER="/etc/systemd/system/slm-copilot-lb-healthcheck.timer"
...
#!/usr/bin/env bash
# Auto-generated by SLM-Copilot -- deregisters this VM from remote LB on shutdown
...
[Unit]
Description=SLM-Copilot LB Deregistration
...
UNITEOF
        ...
        systemctl enable slm-copilot-lb-deregister.service
        systemctl start slm-copilot-lb-deregister.service
    fi
}
...
#!/usr/bin/env bash
# Auto-generated by SLM-Copilot -- removes dead backends from LiteLLM LB
set -o pipefail
...
HEALTH_TIMEOUT=10
MAX_FAILURES=3
STATE_DIR="/var/lib/slm-copilot/healthcheck"
LOG="/var/log/one-appliance/slm-copilot.log"
...
[Unit]
Description=SLM-Copilot LB Backend Health Check
After=slm-copilot-proxy.service
...
[Unit]
Description=SLM-Copilot LB Backend Health Check Timer
...
TIMEREOF
    ...
    systemctl daemon-reload
    systemctl enable slm-copilot-lb-healthcheck.timer
    systemctl start slm-copilot-lb-healthcheck.timer
...
service_bootstrap() {
    ...
    if is_lb_mode; then
        ...
        systemctl stop slm-copilot-lb-healthcheck.timer 2>/dev/null || true
        systemctl disable slm-copilot-lb-healthcheck.timer 2>/dev/null || true
    fi
