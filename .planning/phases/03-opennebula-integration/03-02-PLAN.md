...
must_haves:
  ...
  artifacts:
    - path: "appliances/slm-copilot/appliance.sh"
      provides: "write_report_file helper called from service_bootstrap, SSH banner installed via inline heredoc in service_install, service_help updated with report/log/banner paths"
  ...
  key_links:
    ...
    - from: "service_install()"
      to: "/etc/profile.d/slm-copilot-banner.sh"
      via: "inline heredoc writes banner script during Packer build"
...
Output: Updated appliance.sh with write_report_file helper and inline banner installation, new marketplace.yaml file.
</objective>
...
<task type="auto">
  <name>Task 1: Add write_report_file helper, SSH banner installation, and service_help updates</name>
  <files>appliances/slm-copilot/appliance.sh</files>
  <action>
**A. Add write_report_file() helper** to appliance.sh (new REPORT section, after the logging helpers):
...
**B. Install the SSH banner via inline heredoc in service_install()** -- add BEFORE the "Set ownership" step (before `chown -R`). Use an inline heredoc (the one-apps framework sources appliance.sh, so `$0` is unreliable -- inline is the robust approach):
...
**C. Wire write_report_file into service_bootstrap()** -- call it at the END, after the `attempt_letsencrypt` call and before the final completion log line. This ensures service status fields reflect actual running state (Pitfall 2).

**D. Update service_help()** to add report file and log file paths to the help text. Add after the "Configuration files:" section:
```
Report and logs:
...
**E. Shellcheck compliance** -- run shellcheck on appliance.sh and ensure zero warnings from the new code.
  </action>
  <verify>
...
- `grep 'Cline JSON' appliances/slm-copilot/appliance.sh` shows JSON snippet in report file
  ...
  <done>
write_report_file helper exists and is called from service_bootstrap (after services running). Report includes endpoint, credentials, model, status, Cline setup instructions, JSON snippet, and curl test command. Banner script installed to /etc/profile.d/ via inline heredoc during service_install. service_help updated with report/log paths. Script passes shellcheck.
...
7. `grep 'BANNER_EOF' appliances/slm-copilot/appliance.sh` -- shows inline heredoc
...
- SSH banner installed to /etc/profile.d/ via inline heredoc in service_install, shows live service status on login
